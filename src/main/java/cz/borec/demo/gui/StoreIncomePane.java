...
public class StoreIncomePane extends AbstractPaneBase {
	...
	private TextField textFieldSupplierCompany;

	public StoreIncomePane(Controller controller) {
		super(controller);
	...
	protected Node createMainContent() {
		...
		label1 = new Label("Cena:");
		textFieldPrice = new TextField ();
		grid.add(label1, 0, 2);
		grid.add(textFieldPrice, 1, 2, 2, 1);
		
		label1 = new Label("Dodavatel:");
		textFieldSupplierCompany = new TextField ();
		grid.add(label1, 0, 3);
		grid.add(textFieldSupplierCompany, 1, 3, 2, 1);
	...
	protected void createButtons(HBox hbox) {
		...
		buttonOK.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				...
				controller.performStoreIncome(product, amount, price, textFieldSupplierCompany.getText());
			}
		});
